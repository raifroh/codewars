...
//return an array of objects with username added
//example see left
//psuedo code = grab the first and last name
//append and make all lowercase
//remove period
//append 2023 - age
...
const addUsername = list => {
  //I think a map will work
  let answer = list.map(obj => {
    const currentYear = new Date().getFullYear()
    const birthYear = currentYear - obj.age;
    const username = `${obj.firstName.toLowerCase()}${obj.lastName.toLowerCase().replace('.','')}${birthYear}` 
   return {...obj, username} 
  }) 
  return answer
}
